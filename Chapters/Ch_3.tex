...
The aim pursued in this thesis is to apply machine learning methods, specifically \textit{Principal Component Analysis} (PCA) and an \textit{Artificial Neural Network} (ANN), employing Python tools, on a dataset consisting of hyperspectral Raman maps acquired from
patient-derived head and neck squamous cell carcinoma (HNSCC) biopsies, already prepared for computation. Through these methods, the composition of the available data is investigated, with particular interest in evaluating a binary classification pipeline to identify a defined tumor class among different tissue types.
