...
The analyzed biological tissue consisted of patient-derived head and neck squamous cell carcinoma (HNSCC). It was taken from 25 \textit{samples}, one from each of 25 different patients, so that samples and patients are in one-to-one correspondence in this dataset.

Two to five areas, called \textit{maps}, were then selected from each sample for the Raman spectra acquisition.
A spectrum was acquired for each pixel in the map. Each map has a size of 120$\times$120 pixels, obtained by discarding the outermost row and column of the original 121$\times$121 acquisition to remove photobleaching artefacts.
...
\begin{table}[ht]
    ...
    \begin{tabular}{|c|l|r|r|}
        ...
        \hline
    \end{tabular}
    \caption{Per-class composition of the dataset, ordered by abundance. The \textit{unlabeled/transparent} and \textit{artifact} classes are non-tissue classes that are excluded before further analysis (see text).}
...
Before being provided for this work, the spectra had undergone a standard Raman preprocessing pipeline, including cosmic-ray (spike) removal, subtraction of the quartz-substrate contribution and baseline correction to remove the autofluorescence background. For the cross-class comparisons reported below, the averaged spectra were further normalized with the standard normal variate (SNV) transform, which centres each spectrum and scales it to unit standard deviation, so that band shapes can be compared independently of overall intensity.

The spectra, originally acquired over the \texttt{[acquisition range to confirm]}~$\mathrm{cm}^{-1}$ interval, have been stripped of the ``silent region'' (1800--2800 $\mathrm{cm}^{-1}$), where no useful biological information is found; this leaves the 483 bands described above, which span the fingerprint and CH regions.
% TODO: replace [acquisition range to confirm] with the exact range (in cm^-1) obtained from the supervisor.
% Note: the earlier "200--3500 cm^-1" / "30%" figures came from a different (master-thesis) interpolation grid and are unverified for this dataset.
% --- FALLBACK if the exact range cannot be obtained: comment out the paragraph above and uncomment the line below. ---
...
% If performance stays flat down to a small fraction, replace the sentence above with the measured number (e.g. "down to X% with no significant performance loss") and add the plot as a figure. This reduces the possible choices for \textit{healthy} labels to the most numerous ones, i.e.\ \textit{tumor stroma (6)}, \textit{stroma with inflammation (4)} and \textit{connective tissue (5)} (\Cref{fig:label_distribution}).

Regarding distinguishability, the most similar tissues are also the most clinically confusable. Being able to discriminate would then be the most valuable result. The three possible healthy classes show a comparable degree of similarity to the tumor (\Cref{fig:spectra_comparison_similar}), each then constitutes an equally valid choice, with no class being a priori better suited than the rest from a separability standpoint.
