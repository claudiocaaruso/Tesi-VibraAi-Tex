...
Biological tissues were divided in 14 classes, the most represented of which was the \textit{tumor} class (47,92\%). Occurrences for each class are shown in Fig.\ref{fig:label_distribution}


Elimination of non relevant spectra for the dataset (i.e. unlabeled/transparent and artifacts) and binary grouping in tumoral/healty balances out the dataset (Fig.\ref{fig:binary_label_distribution})
...
All classes appear to have the same trends, with multiple similar peaks in the 'fingerprint region' (500-1800 $cm^{-1}$) and a major peak in the 'CH region' (1800-3500 $cm^{-1}$). The only exceptions are represented by \textit{unlabeled/transparent}, \textit{artifact} and \textit{necrosis} pixels, which noticeably differed from all others (Fig.\ref{fig:spectra_comparison_discarded})
