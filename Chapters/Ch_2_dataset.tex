...
Due to the class imbalance (tumor alone being 47\% of the dataset) the classes will not be equal in numerosity anymore, requiring rebalancing by discarding part of the available training data. The model is then trained on a reduced number of spectra. Due to the size of the dataset (>1000000 spectra) and the similarity of the spectra linked to adjacent pixels, the subsampling can be performed without major shortcomings down to 5-10\% of the original dataset (with the constraint of keeping spectra from each sample). This reduces the possible choices for \textit{healthy} labels to the most numerous ones, i.e. \textit{tumor stroma (6)}, \textit{stroma with inflammation (4)} and \textit{connective tissue}(5))(\Cref{fig:label_distribution}).
...
Classification will then be performed on the tumor vs healthy approach to assess results to the global problem, while the Tumor vs. Tumor stroma as a surgically oriented problem, with a spectrally homogeneous training set based on the most numerous healthy class, expecting for it better results overall.
...
\begin{figure}[H]
    ...
    \caption{Maps with highlighted labels based on classification approach. \textit{Stroma with inflammation} was chosen over \textit{Tumor stroma}, although not being the final choiche for training, for representative purposes. No map in the dataset contained both \textit{tumor stroma} and other labels.}
