...
The classification stage was based on three model architectures (S, M, L) that varied in width (\cref{tab:architectures}). A grid search was performed by varying model size and multiple hyperparameters to determine the best possible combination. Too small of a model would be expected to underfit, not learning enough relevant patterns to be able to categorize well enough, while too big would overfit, memorizing the dataset rather than generalizing.
...
The models all consisted of \textit{dense} (fully connected) layers, with \textit{dropout} rate of 0.4 to reduce overfitting. \textit{Dropout} is a regularization technique such that at every training step, every neuron (excluding the output neurons) has a probability p of being temporarily ignored, though it may be active during the next step. Neurons trained with \textit{dropout} cannot co-adapt with their neighboring neurons; they have to be as useful as possible on their own. They also cannot rely excessively on just a few input neurons and must pay attention to each of their input neurons \cite{geron2022hands}.

All the hidden layers used ReLU as activtion functions and the number of input units is set to the number of retained PC components, while the output neurons had a sigmoid activation.

The best result was observed for the small (S) architecture, yielding similar results to the medium (M) one but with lesser computational cost and risk of overfitting, while the large (L) model showed clear overfitting.


\section{Data Splitting And Cross Validation}
\label{sec:data_splitting}

The spectra in the dataset are spatially correlated: neighbouring pixels show similar patterns and peaks. Therefore, a simple random split of the whole dataset in training, validation and test sets would result in spectra from the same map or sample to be present in all the divisions. Such a split would let the model memorize sample specific signatures and give misleading results due to the data leakage in the test set.

Furthermore, due to the difference across maps and samples regarding the presence of different biological tissues, the performance of the model in testing and validation is heavily dependent on the samples present in the sets.

A splitting protocol based on Sample IDs was thus impemented to solve the two problems:
\begin{itemize}
    \item \textit{Data splitting} in train/validation/test so that results were realistic and trustworthy.
    \item \textit{Cross validation} on multiple test sets in order to obtain an average and well representative result.
\end{itemize}

Regarding data splitting, the Sample IDs are considered when performing the train/validation/test split in order to prevent the same sample from appearing in both training and testing. A stratified split is performed while keeping this constraint and mantaining the ratio of the two labels close to 1:1 during training. 


\section{Spatial post-processing}
\label{sec:spatial_post_processing}

The network outputs one probability for each spectrum. Since spectra are acquired on a
regular Raman map, each probability is associated with spatial coordinates \((x,y)\).
Adjacent pixels correspond to spatially close measurements and are therefore likely to
contain related biological information. 

Spatial post-processing is applied after the network has produced its predicted
probabilities, without modifying the architecture, the training procedure or the
learned parameters. In particular, smoothing is applied to the continuous class-1
probabilities before converting them into binary labels through the classification
threshold. The operation is performed independently within each Raman map, identified by
\texttt{Sample\_ID} and \texttt{Map\_ID}. Borders are handled by replacing the missing neighbouring values with the nearest
available pixel values of the same map.

Let \(p(x,y)\) be the probability predicted by the model for the positive class at the
pixel located at coordinates \((x,y)\). With a \(k \times k\) mean filter, the smoothed
probability \(\tilde{p}(x,y)\) can be written as
\begin{equation}
    \tilde{p}(x,y) =
...
\end{equation}
where \(\mathcal{N}_{k \times k}(x,y)\) is the local neighbourhood centered on
\((x,y)\). The kernel size \(k\) is a configurable parameter; in the experiments,
different neighbourhood sizes, such as \(3 \times 3\) and \(5 \times 5\), were considered.

After smoothing, the final binary prediction is obtained by applying the same
classification threshold used for the raw probabilities:
\begin{equation}
    \hat{y}(x,y) =
    \begin{cases}
        1, & \tilde{p}(x,y) > \tau,\\
...
\end{equation}
where \(\tau = 0.47\) in the experiments considered in this work. The objective is to
reduce isolated noisy predictions and obtain spatially more coherent probability maps.
This operation can also remove small true regions or propagate local errors; for this
reason, raw and smoothed predictions are compared separately in the experimental results.
