...
The network classifies each pixel in isolation, learning to recognize its composition solely from the corresponding spectrum and not accounting for spatial structure. However, since the spectra are acquired within maps, they are also associated with spatial coordinates \((x,y)\) and consequently adjacent pixels are correlated, as they often belong to the same biological region.
...
To introduce the spatial classification problem and test if these correlations are actually relevant in order to improve predictions, a spatial filter is used. Starting from the probabilities predicted by the model for each pixel, this filter calculates the average on a spatial window of arbitrary size, correcting it on the basis of the context of the predicted neighbouring probabilities.
This step does not alter the neural network's behavior or intrinsic capacity, since spatial information never enters the network's training phase and is applied only after inference.
...
After smoothing, the probabilities are converted into binary labels based on the \textit{decision threshold}:
