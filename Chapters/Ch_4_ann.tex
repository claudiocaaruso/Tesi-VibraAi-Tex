...
The best result was observed for the small (S) architecture, yielding similar results to the medium (M) one but with lower computational cost and risk of overfitting, while the large (L) model showed clear overfitting.
...
where \(\tau = 0.47\) in the experiments considered in this work. The objective is to reduce isolated noisy predictions and obtain spatially more coherent probability maps. This operation can also remove small true regions or propagate local errors; for this reason, raw and smoothed predictions are compared separately in the experimental results.
...
Thie tradeoff is always present: the higher the recall (TPR), the more false positives (FPR) the classifier produces. The dashed line in \cref{fig:roc_curve} represents the ROC curve of a purely random classifier; a good classifier stays as far away from that line as possible (toward
the top-left corner).
