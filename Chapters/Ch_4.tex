...
Hyperspectral maps were acquired using a home-built Raman microspectroscopy system. The excitation source used is a monochromatic, continuous-wave, diode-pumped laser operating in the red spectral region, specifically around \(660\,\mathrm{nm}\) (exposure time of \(500\,\mathrm{ms}\) and \(120\,\mathrm{mW}\) laser power).
...
The Rayleigh component has been removed because, since its intensity is several orders of magnitude greater than that associated with Raman scattering, it represents a disturbance that could hide the weakest signal. The acquired spectra were calibrated and represented as a function of intensity versus Raman shift measured in \(\mathrm{cm}^{-1}\).
