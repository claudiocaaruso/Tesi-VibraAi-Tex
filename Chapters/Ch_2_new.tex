...
where \(n\) is the number of observations and \(p\) is the number of variables. In this work, each observation is a Raman spectrum and \(p = 483\), as described in \Cref{sec:origin-structure-ch4}.
...
Although it will speed up training, reducing dimensionality does lose some information, so it cannot be said that it automatically leads to an improvement. Its effect must be evaluated in terms of downstream classification performance.
...
Let \(\mathbf{X} \in \mathbb{R}^{n \times p}\) be the data matrix, where each row \(\mathbf{x}_i^{\mathrm{T}}\) is one observation and each column is one of the \(p\) variables. PCA assumes that the dataset is centred around the origin. The mean vector is computed as
...
where \(\mathbf{1}\) is a column vector of ones. Centring the data means subtracting the average vector from each observation, so that the variations are described around an average profile and not at an absolute level.
...
The \textit{principal directions} can be obtained through the Singular Value Decomposition (SVD) \cite{brunton2019datadriven} of the centred data matrix. The SVD is a standard matrix factorisation technique that decompose \(\mathbf{X}_c\) in:
\begin{equation}
    \mathbf{X}_c =
...
The principle underlying dimensionality reduction is summarised in the following rank-\(d\) approximation:
...
In this way, the original dataset was decomposed and subsequently reconstructed using a smaller number \(d\) of directions instead of all the initial variables \(p\). 
...
The metrics analysed in order to make this assessment are the \textit{Explained Variance Ratio} (EVR), which is the variance fraction explained by each \(\mathrm{PC}_j\) component, strictly linked to the singular values
