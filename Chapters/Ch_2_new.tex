...
where \(n\) is the number of observations and \(p\) is the number of variables. Each observation can therefore be interpreted not only as a curve or signal, but also as a point in a \(p\)-dimensional feature space, where each coordinate corresponds to one variable. In this work, each observation is a Raman spectrum and \(p = 483\), as described in \Cref{ch:Dataset}.
