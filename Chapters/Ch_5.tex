...
The comparison of the tested configurations showed that the best trade-off was obtained with the small architecture (S), \(50\) retained PCA components and a batch size of \(8192\). This configuration achieved the most favourable behaviour according to the evaluation metrics introduced in \Cref{sec:evaluation_metrics-ch4}, while keeping both the model and the input representation relatively compact.

The comparison between architectures also showed that larger models did not provide a clear advantage for the present dataset. Although some configurations with more components or larger networks could still obtain good numerical scores, their training curves showed less stable behaviour and a larger discrepancy between training and validation performance. The small model was therefore preferred because it provided a more regular learning behaviour with lower computational cost and reduced risk of overfitting.
...
The maximum number of epochs was set to \(100\), with early stopping based on validation AUC and learning-rate reduction on plateau, as described in \Cref{sec:training-ch4}.

The classification threshold used to convert the predicted probability into a binary label was selected empirically by comparing threshold values between \(0.45\) and \(0.50\). The final value was set to \(\tau=0.49\), slightly below the standard threshold of \(0.50\), as it provided a practical compromise between favouring recall for the positive class and maintaining balanced overall performance. In this application, a false negative, corresponding to a tumour-related spectrum classified as non-tumour, is considered more critical than a false positive, corresponding to a non-tumour spectrum classified as tumour-related. A slightly lower threshold increases the tendency of the classifier to assign spectra to the positive class, thereby supporting sensitivity to tumour-related spectra without excessively penalizing accuracy.
...
\textbf{Writing notes.} This section should describe how the selected ANN configuration behaves during training for the two final experiments. The two experiments can be discussed together, using the same criteria for comparison, rather than creating two separate subsections.

\begin{itemize}
    \item Insert the training curves for both experiments, preferably with loss, accuracy and AUC shown for training and validation sets.
    \item Comment on convergence: whether the curves stabilize and after how many epochs the main improvement occurs.
    \item Compare training and validation trends to identify possible overfitting or instability.
    \item Point out whether one experiment appears more stable or more difficult to train.
    \item If useful, mention that the final model is selected according to validation behaviour, consistently with the training protocol described in \Cref{sec:training-ch4}.
\end{itemize}
