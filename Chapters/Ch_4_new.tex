...
Since the absolute counts (TP, FP, TN, FN) can be very big numbers when dealing with large datasets, another way to represent the confusion matrix is through the classification rates (\cref{fig:confusion_matrix_rates-ch4}), defined hereafter.
...
\begin{figure}[ht]
    \centering
    \subfloat[Confusion matrix expressed as absolute counts.\label{fig:confusion_matrix-ch4}]{
    ...
    \hfill
    \subfloat[Confusion matrix expressed as classification rates.\label{fig:confusion_matrix_rates-ch4}]{
        \begin{tikzpicture}[font=\small]
            \draw (0,0) rectangle (2,2);
            \draw (2,0) rectangle (4,2);
            \draw (0,-2) rectangle (2,0);
