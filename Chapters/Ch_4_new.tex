...
After acquisition, the raw Raman spectra underwent a stage of preprocessing which was not part of the work for this thesis, but will be briefly illustrated for completeness.
...
The spectra, acquired over a broader Raman-shift range, have been stripped of the \textit{silent region} (1800--2800 $\mathrm{cm}^{-1}$) (\cref{sec:raman_biological_tissues}), where no useful biological information is found, reducing the number of bands from 888 to 483. The remaining \textit{fingerprint} and \textit{CH} regions were normalized separately and then joined.
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=0.65\linewidth]{Images/Images_ch_2/spectrum_avg_tumor.png}
...
\begin{figure}[H]
    ...
    \subfloat[\textit{Tumor} vs. \textit{healthy} classification (1st approach).\label{fig:heatmap-tumor-vs-healthy-ch4}]{
        \includegraphics[width=0.45\textwidth]{Images/Images_ch_2/heatmap_tumor_vs_healthy.png}
    }
    \hfill
    \subfloat[\textit{Tumor} vs. \textit{stroma with inflammation} classification (2nd approach).\label{fig:heatmap-tumor-vs-stroma-ch4}]{
        \includegraphics[width=0.45\textwidth]{Images/Images_ch_2/heatmap_tumor_vs_stroma.png}
    }
...
\begin{table}[ht]
    ...
    \begin{tabular}{|l|c|c|c|c|}
        ...
        \hline
        Small (S)  & 2 & 16, 8      & 0.2 (just for first layer) & 1 (sigmoid) \\
        Medium (M) & 2 & 64, 32      & 0.2 & 1 (sigmoid) \\
        Large (L)  & 3 & 128, 64, 16 & 0.2  & 1 (sigmoid) \\
        \hline
    \end{tabular}
...
The models all consisted of \textit{dense} (fully connected) layers, with a \textit{dropout} rate of 0.2 to reduce overfitting, except for the S model, where \textit{dropout} was applied just to the first layer due to the small size of the network. \textit{Dropout} is a regularization technique such that at every training step, every neuron in a dropout layer has a probability $p=0.2$ of being temporarily ignored, though it may be active during the next step. Neurons trained with \textit{dropout} cannot co-adapt with their neighboring neurons; they have to be as useful as possible on their own. They also cannot rely excessively on just a few input neurons and must pay attention to each of their input neurons \cite{geron2022hands}.

All the hidden layers used ReLU as activation functions and the number of input units is set to the number of retained PC components (itself a hyperparameter, selected jointly with the architecture, see \cref{sec:training-ch4}), while the output neurons had a sigmoid activation.
...
Spatial post-processing is applied after the network has produced its predicted probabilities, without modifying the architecture, the training procedure or the learned parameters. In particular, smoothing is applied to the probabilities before converting them into binary labels through a \textit{decision threshold} $\tau$ (its role in the precision/recall tradeoff is discussed in \cref{sec:evaluation_metrics-ch4}). Borders are handled by replacing the missing neighbouring values with the nearest available pixel values of the same map.

Let \(p(x,y)\) be the probability predicted by the model for the positive class at the pixel located at coordinates \((x,y)\). With a \(w \times w\) mean filter, the smoothed probability \(\tilde{p}(x,y)\) can be written as
...
The kernel size $w$ (not to be confused with the number of cross-validation folds $k$, \cref{sec:data_splitting-ch4}) is a configurable parameter; in the experiments, two neighbourhood sizes were considered: \(3 \times 3\) and \(5 \times 5\).
...
The tradeoff is always present: the higher the recall (TPR), the more false positives (FPR) the classifier produces. The dashed line in \cref{fig:roc_curve-ch4} represents the ROC curve of a purely random classifier; a good classifier stays as far away from that line as possible (toward
the top-left corner).
